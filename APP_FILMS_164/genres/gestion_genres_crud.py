...
"""
    ...
    Remarque :  Dans le champ "nom_genre_delete_wtf" du formulaire "genres/computer_delete_wtf.html",
    ...
    try:
        ...
        if request.method == "POST" and form_delete.validate_on_submit():
            ...
            if form_delete.submit_btn_conf_del.data:
                ...
                # le formulaire "genres/computer_delete_wtf.html" lorsque le bouton "Etes-vous sur d'effacer ?" est cliqué.
            ...
            if form_delete.submit_btn_del.data:
                ...
                str_sql_delete_films_genre = """DELETE FROM t_genre_film WHERE fk_genre = %(value_id_computer)s"""
        ...
        if request.method == "GET":
            ...
            str_sql_computer_note_delete = """SELECT * FROM t_computer_note 
                                            INNER JOIN t_note ON t_computer_note.fk_note = t_note.idNote
                                            INNER JOIN t_computer ON t_computer_note.fk_computer = t_computer.idComputer
                                            WHERE fk_computer = %(value_id_computer)s"""

            with DBconnection() as mydb_conn:
                mydb_conn.execute(str_sql_computer_note_delete, valeur_select_dictionnaire)
                data_notes_attribue_computer_delete = mydb_conn.fetchall()
                print("data_films_attribue_computer_delete...", data_notes_attribue_computer_delete)

                # Nécessaire pour mémoriser les données afin d'afficher à nouveau
                # le formulaire "genres/computer_delete_wtf.html" lorsque le bouton "Etes-vous sur d'effacer ?" est cliqué.
                session['data_films_attribue_computer_delete'] = data_notes_attribue_computer_delete

                # Opération sur la BD pour récupérer "id_genre" et "intitule_genre" de la "t_genre"
                str_sql_id_computer = "SELECT idComputer, comHostname FROM t_computer WHERE idComputer = %(value_id_computer)s"

                mydb_conn.execute(str_sql_id_computer, valeur_select_dictionnaire)
                # Une seule valeur est suffisante "fetchone()",
                # vu qu'il n'y a qu'un seul champ "nom genre" pour l'action DELETE
                data_nom_computer = mydb_conn.fetchone()
                print("data_nom_computer ", data_nom_computer, " type ", type(data_nom_computer), " computer ",
                      data_nom_computer["comHostname"])

            # Afficher la valeur sélectionnée dans le champ du formulaire "computer_delete_wtf.html"
            form_delete.nom_computer_delete_wtf.data = data_nom_computer["comHostname"]

            # Le bouton pour l'action "DELETE" dans le form. "computer_delete_wtf.html" est caché.
            btn_submit_del = False

    except Exception as Exception_genre_delete_wtf:
        raise ExceptionComputerDeleteWtf(f"fichier : {Path(__file__).name}  ;  "
                                      f"{computers_delete_wtf.__name__} ; "
                                      f"{Exception_genre_delete_wtf}")

    return render_template("computers/computer_delete_wtf.html",
                           form_delete=form_delete,
                           btn_submit_del=btn_submit_del,
                           data_computer_associes=data_notes_attribue_computer_delete)
